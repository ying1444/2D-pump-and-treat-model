...
dy=20;
dt=20; %should be somewhat less than dx/seepage_Q for stability of code
...
while   any(any(C(:,1:n-1,t_increment)>10*28.3))>0 %while concentrations anywhere are above 10 ug/L keep running through loops
    ...
    for j=2:1:n-1 
        ...
        if j==n-1;
                ...
                %sacme as numbers as a colume before
            for i=2:1:m-1
    ...
    Average_C(t_increment)=mean(mean(C(:,1:n-6,t_increment))); %checking average concentration levels in pue     
...
plot(total_C);
plot(Average_C);        
title('Average Concentraton 10 Year Plume')
xlabel('X distance (50 ft)') 
ylabel('Y distance (20 ft)') 
